...
export function TimezoneSelect({ value, onChange, disabled = true }: TimezoneSelectProps) {
  return <div className={eventUi.field}>
    <label className={eventUi.label} htmlFor="event-timezone">Timezone</label>
    <select className={eventUi.input} disabled={disabled} id="event-timezone" onChange={(event) => onChange(event.target.value)} value={value}>
